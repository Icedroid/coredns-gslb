...
*gslb* - returns your resolver's local IP address, port and transport.

## Description

The *gslb* plugin is not really that useful, but can be used for having a simple (fast) endpoint
to test clients against. When *gslb* returns a response it will have your client's IP address in
...
The *gslb* plugin will respond to every A or AAAA query, regardless of the query name.
...
Start a server on the default port and load the *gslb* plugin.
...
[code]: https://github.com/coredns/coredns/blob/master/plugin/gslb/
